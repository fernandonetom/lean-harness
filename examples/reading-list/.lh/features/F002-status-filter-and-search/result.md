...
Feature F002 (Status filter and search) needs fixes.
...
- ../../src/context/task-context.ts -> ../../packages/cli/src/context/task-context.ts (modified)
...
- ../../.changeset/fix-claude-code-cwd-flag.md (created)
- ./ (created)
- ../../scripts/sync-example-hosts.mjs (created)

## Acceptance Criteria Result

- [x] AC1: **AC-01:** The list page (`/`) supports a status filter control (e.g. a select/links for `all`, `to-read`, `reading`, `done`) that, when set, shows only items whose `status` matches the selected value. — pass
- [x] AC2: **AC-02:** The list page supports a text search input that, when given a keyword, shows only items whose `title` contains that keyword (case-insensitive substring match). — pass
- [x] AC3: **AC-03:** Status filter and text search can be combined (both applied together as an AND). — pass
- [x] AC4: **AC-04:** When no filter/search is applied, all items are shown (existing behavior is preserved). — pass
- [x] AC5: **AC-05:** Filtering is implemented client-side or via URL `searchParams` on the existing list page — no changes to `web/app/api/items/route.ts`, `web/lib/store.ts`'s on-disk JSON persistence format, or `web/data/reading-list.json` are required. — pass
- [x] AC6: **AC-06:** `pnpm --dir web run lint` and `pnpm --dir web run build` both succeed with no errors after the change. — pass
...
- `opencode run --agent lh-builder ...`: skipped — skipped: command not recognized as safe and not explicitly provided
...
- ../../.changeset/fix-claude-code-cwd-flag.md
- ./
...
- `opencode run --agent lh-builder ...` — skipped
