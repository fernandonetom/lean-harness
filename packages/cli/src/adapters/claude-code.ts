...
export function buildClaudeCodeArgs(input: AgentRunInput): string[] {
  ...
  // Note: the working directory is set via the child_process `cwd` spawn option
  // (see runClaudeCode below), not a CLI flag — the installed `claude` CLI has no
  // `--cwd` option and rejects it with "error: unknown option '--cwd'".
