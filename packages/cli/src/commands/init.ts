...
import { loadOpenCodePluginFiles, loadOpenCodePluginPackageSpec } from "./load-opencode-plugins.js";
...
export interface InitOptions {
  ...
  noPin?: boolean;
  /**
   * Restore the pre-v2.x behavior of copying the guardrail plugin's raw JS files into
   * .opencode/plugins/, instead of the default of registering the npm-published
   * @feneto/lh-opencode package in opencode.json's "plugin" array. For offline/air-gapped/
   * restricted environments where OpenCode's Bun-based npm-plugin auto-install isn't viable.
   */
  localPlugin?: boolean;
}
...
interface InitResult {
  ...
  files: Record<string, "created" | "updated" | "skipped" | "removed">;
  warnings: string[];
}
...
export async function runInitCommand(options: InitOptions): Promise<void> {
  const { cwd, force = false, json = false, host, yes = false, global: isGlobal = false, team = false, localPlugin = false } = options;
  ...
  if (hostSelection.openCode) {
    ...
    const ocResult = await installOpenCodePack(cwd, overwrite, log, json, localPlugin);
...
async function installOpenCodePack(
  ...
  const configStatus = await installOpenCodeConfig(cwd, force, log, json, localPlugin);
  ...
  // --- guardrail plugin ---
  if (localPlugin) {
    // Legacy/offline path: copy the raw guardrail JS files into .opencode/plugins/. The default
    // path instead registers the npm-published @feneto/lh-opencode package via
    // installOpenCodeConfig()'s "plugin" array — see docs/hosts/opencode.md.
    ...
    // Default (npm-registered) mode. LeanHarness-managed local plugin files left over from a
    // prior --local-plugin run (or a pre-v2 install) would double-register the guardrail hooks
    // alongside the npm-published package — OpenCode auto-loads every .js file dropped into
    // .opencode/plugins/, regardless of what's registered in opencode.json. Only remove them
    // under --force (matching this function's overwrite convention elsewhere), and only these
    // exact LeanHarness-managed filenames — never touch other files a user may have added.
    const stalePluginFiles = loadOpenCodePluginFiles().filter((pf) => fs.existsSync(opencodePluginPath(cwd, pf.filename)));
    const npmPluginLabel = loadOpenCodePluginPackageSpec().split("@^")[0];

    if (stalePluginFiles.length > 0 && force) {
      for (const pf of stalePluginFiles) {
        fs.unlinkSync(opencodePluginPath(cwd, pf.filename));
        result.files[`.opencode/plugins/${pf.filename}`] = "removed";
        if (!json) {
          log.info(`  .opencode/plugins/${pf.filename} (removed — superseded by npm-published ${npmPluginLabel})`);
        }
      }
    } else if (stalePluginFiles.length > 0) {
      const names = stalePluginFiles.map((pf) => pf.filename).join(", ");
      result.warnings.push(
        `Local OpenCode plugin files still present at .opencode/plugins/ (${names}) alongside the npm-registered ${npmPluginLabel} plugin — OpenCode will load both, double-registering the guardrail hooks. Re-run with --force to remove the stale local files, or pass --local-plugin if you intend to keep using local files instead of the npm package.`,
      );
      if (!json) {
        log.info(`  .opencode/plugins/ (WARNING: stale local files present — ${names}; double-registration risk, re-run with --force to remove)`);
      }
    } else if (!json) {
      log.info(`  .opencode/plugins/ (skipped — using npm-published ${npmPluginLabel} instead, see opencode.json "plugin". Use --local-plugin to write local files.)`);
    }
  }
...
async function installOpenCodeConfig(
  ...
  const lhConfig = createOpenCodeConfigObject(localPlugin);
...
function mergeOpenCodeConfig(
  ...
  const lhPlugins = lhConfig["plugin"];
  if (Array.isArray(lhPlugins) && lhPlugins.length > 0) {
    const existingPlugins = Array.isArray(result["plugin"]) ? (result["plugin"] as unknown[]) : [];
    const merged = [...existingPlugins];
    for (const spec of lhPlugins) {
      const specName = typeof spec === "string" ? spec.split("@^")[0] : null;
      const alreadyPresent = merged.some((p) => typeof p === "string" && specName && p.split("@^")[0] === specName);
      if (!alreadyPresent) merged.push(spec);
      else if (!force) {
        warnings.push(`OpenCode plugin already registered in opencode.json: ${specName}. Use --force to update the LeanHarness-managed entry.`);
      }
    }
...
function createOpenCodeConfigObject(localPlugin = false): Record<string, unknown> {
  return {
    "$schema": "https://opencode.ai/config.json",
    ...(localPlugin ? {} : { "plugin": [loadOpenCodePluginPackageSpec()] }),
    "permission": {
      "read": "allow",
      "list": "allow",
      "glob": "allow",
