...
export async function runDoctorCommand(options: DoctorOptions): Promise<void> {
  ...
  let ocConfigData: Record<string, unknown> | null = null;
  if (ocConfigExists) {
    try {
      const { readJsonFile } = await import("../core/fs.js");
      ocConfigData = await readJsonFile<Record<string, unknown>>(ocConfigPath);
      checks.push(
        ocConfigData !== null
  ...
  const ocNpmPluginRegistered = Array.isArray(ocConfigData?.["plugin"])
    && (ocConfigData!["plugin"] as unknown[]).some((entry) => typeof entry === "string" && entry.includes("@feneto/lh-opencode"));
  ...
  // --- OpenCode plugin ---
  // v2.0.0+: the guardrail plugin defaults to the npm-published @feneto/lh-opencode package
  // registered in opencode.json's "plugin" array — .opencode/plugins/ is only populated with
  // --local-plugin. Absence of the local files is only a problem if neither distribution mode
  // is configured at all.
  const ocSharedJsExists = await fileExists(opencodePluginPath(cwd, "shared.js"));
  const ocGuardrailsJsExists = await fileExists(opencodeGuardrailPluginPath(cwd));
  const ocLocalPluginPresent = ocSharedJsExists || ocGuardrailsJsExists;

  checks.push(
    ocSharedJsExists
      ? { name: ".opencode/plugins/shared.js", status: "pass", message: "present (--local-plugin mode)" }
      : ocNpmPluginRegistered
        ? { name: ".opencode/plugins/shared.js", status: "pass", message: "not needed — using npm-published @feneto/lh-opencode (see opencode.json \"plugin\")" }
        : { name: ".opencode/plugins/shared.js", status: "warn", message: "missing — run `lh init --host opencode`" },
  );

  checks.push(
    ocGuardrailsJsExists
      ? { name: ".opencode/plugins/leanharness-guardrails.js", status: "pass", message: "present (--local-plugin mode)" }
      : ocNpmPluginRegistered
        ? { name: ".opencode/plugins/leanharness-guardrails.js", status: "pass", message: "not needed — using npm-published @feneto/lh-opencode (see opencode.json \"plugin\")" }
        : { name: ".opencode/plugins/leanharness-guardrails.js", status: "warn", message: "missing — run `lh init --host opencode`" },
  );

  if (ocNpmPluginRegistered && ocLocalPluginPresent) {
    checks.push({
      name: "OpenCode guardrail plugin",
      status: "fail",
      message: "double-registration risk — @feneto/lh-opencode is registered in opencode.json AND local files exist in .opencode/plugins/ (OpenCode auto-loads every .js file there). Likely a leftover from a v1.x/--local-plugin install; remove .opencode/plugins/shared.js and .opencode/plugins/leanharness-guardrails.js, or drop the npm entry from opencode.json's \"plugin\" array.",
    });
  }
