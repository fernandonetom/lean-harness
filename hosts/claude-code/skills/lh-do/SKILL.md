...
- **Use the Agent tool** to delegate each workflow step to the appropriate subagent (always include the required `description` field):
    - Specify step → use `Skill(skill: "lh-spec")` to load the skill directly, or spawn Agent with `description: "Run lh-spec for <feature-id>"` and prompt: `"Follow the lh-spec skill's workflow for: <request>"`.
