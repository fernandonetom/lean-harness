...
- `.lh/memory/cave.md` — **see rules below**

Do not write to implementation code files.

## Memory file rules (`.lh/memory/cave.md`)

When compression produces a new abbreviation not already present in `.lh/memory/cave.md`'s `## Abbreviation Map` section, append it as `- <abbr>: <full form>`. Follow these strict rules:

- **Only append genuinely new, reusable abbreviations.** Do not append per-feature or per-task noise (e.g., do not write lines like "F001 compressed (full, 12 messages)"; that is not an abbreviation).
- **If the `## Abbreviation Map` section does not exist yet, create it** (append a `## Abbreviation Map` heading to the end of `cave.md`, then the new entry beneath it).
- **Hard cap: 60 lines in `cave.md`.** If already at or near the cap, consolidate or drop the least-used entries instead of growing the file further.
