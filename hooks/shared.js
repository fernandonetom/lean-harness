...
function execGitSync(cwd, args) {
  try {
    var out = require('child_process').execFileSync('git', args, {
...
function resolveRoots() {
  ...
  var envRoot = process.env.CLAUDE_PROJECT_DIR || process.cwd();
  var cwd = process.cwd();

  // 1. worktree root — the working tree we are physically inside
  var top = execGitSync(cwd, ['rev-parse', '--show-toplevel']);
  var worktreeRootPath = top ? path.resolve(top) : path.resolve(envRoot);

  // 2. harness root — the MAIN working tree, which owns .lh/state.json and .lh/features/
  var harnessRootPath = worktreeRootPath;
  var common = execGitSync(cwd, ['rev-parse', '--path-format=absolute', '--git-common-dir']);
  if (!common) {
    var rel = execGitSync(cwd, ['rev-parse', '--git-common-dir']);
    if (rel) common = path.resolve(cwd, rel);
  }
  if (common) {
    common = path.resolve(common);
    if (path.basename(common) === '.git') {
      var candidate = path.dirname(common);
      if (dirExists(path.join(candidate, '.lh'))) harnessRootPath = candidate;
    }
  }
  // 3. last resort — trust the env var if it is the only place .lh/ lives
  if (!dirExists(path.join(harnessRootPath, '.lh')) && dirExists(path.join(envRoot, '.lh'))) {
    harnessRootPath = path.resolve(envRoot);
  }

  _rootCache = {
    worktreeRoot: worktreeRootPath,
    harnessRoot: harnessRootPath,
    isLinkedWorktree: path.resolve(worktreeRootPath) !== path.resolve(harnessRootPath)
  };
  return _rootCache;
}

/** Root that owns `.lh/` — state, config, policies, boundaries. */
function projectRoot() { return resolveRoots().harnessRoot; }
/** Root that owns the working files being edited — used for boundary path normalization. */
function worktreeRoot() { return resolveRoots().worktreeRoot; }
function _resetRootCache() { _rootCache = null; }
...
function normalizeAgainstRoots(candidate) {
  var roots = resolveRoots();
  var rel = normalizeRelativePath(roots.worktreeRoot, candidate);
  if (rel && rel.charAt(0) !== '/' && rel.indexOf('__PARENT_ESCAPE__') !== 0) return rel;
  if (roots.isLinkedWorktree) {
    var rel2 = normalizeRelativePath(roots.harnessRoot, candidate);
    if (rel2 && rel2.charAt(0) !== '/' && rel2.indexOf('__PARENT_ESCAPE__') !== 0) return rel2;
  }
  return rel;
}
...
function extractToolPaths(input) {
  ...
  for (var i = 0; i < candidates.length; i++) {
    if (typeof candidates[i] === 'string' && candidates[i]) {
      paths.push(normalizeAgainstRoots(candidates[i]));
    }
  }
  ...
  if (Array.isArray(ti.files)) {
    for (var j = 0; j < ti.files.length; j++) {
      var f = ti.files[j];
      if (typeof f === 'string') paths.push(normalizeAgainstRoots(f));
      else if (f && typeof f.path === 'string') paths.push(normalizeAgainstRoots(f.path));
      else if (f && typeof f.file_path === 'string') paths.push(normalizeAgainstRoots(f.file_path));
    }
  }
  ...
  if (Array.isArray(ti.edits)) {
    for (var k = 0; k < ti.edits.length; k++) {
      var e = ti.edits[k];
      if (e && typeof e.file_path === 'string') paths.push(normalizeAgainstRoots(e.file_path));
      else if (e && typeof e.path === 'string') paths.push(normalizeAgainstRoots(e.path));
    }
  }
...
module.exports = {
  ...
  _resetRootCache: _resetRootCache,
